		...
		,	onSuccess: function(res) {
				...
				if(!res.error) {
					if(!cfsSaveWithoutPreviewUpdate)
						cfsRefreshPreview();
						setTimeout(function(){
							jQuery("#cfsFormPreviewFrame").contents().find('head').append('<link rel="stylesheet" href="'+frontendStyles['cfs.frontend.forms']+'" type="text/css" />');
							jQuery("#cfsFormPreviewFrame").contents().find('head').append('<link rel="stylesheet" href="'+frontendStyles['supTablesUi']+'" type="text/css" />');
						}, 1000);
				}
				cfsSaveWithoutPreviewUpdate = false;
				if(cssSet && cssEditor) {
